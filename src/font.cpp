...
void Font::RenderWithMeassures(const String & text, double x, double y, double w, double h) const {
	int length = text.Length();
	uint32 ret = x;
	for (int i = 0; i < length; i++) {
		Renderer::Instance().DrawImage(this, ret, y, text[i], w, h);
		ret += glyphs[text[i]].GetWidth() + w;
	}
}
