...
/*
 *	Acordarse de que hay parámetros que se pueden cambiar. 
 */
uint32 Renderer::GenImage(uint8 * buffer, uint16 width, uint16 height) const
{
	uint32 gltex; // o GLuint
	glGenTextures(1, &gltex); // Generación textura
	glBindTexture(GL_TEXTURE_2D, gltex); // Activación textura. También vale: Renderer::BindImage(gltex); 
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST); // Parámetros REDUCCIÓN textura
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST); // Parámetros AMPLIACIÓN textura
