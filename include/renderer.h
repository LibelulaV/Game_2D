...
public:
	...
	void DrawTexWithMeassures(const Font * font, const String & text, double x, double y, double w, double h) const;

	virtual void Renderer::DrawTriangle(double x, double y, double width, double height) const;

	virtual void Renderer::GetTextImage(void * buffer) const;

	virtual uint32 Renderer::GenImage(uint8* buffer, uint16 width, uint16 height) const;
	virtual void UpdateImage(uint8 * buffer, uint16 width, uint16 height) const;
	virtual void Renderer::BindImage(uint32 glhandle) const;
	virtual void Renderer::DeleteImage(uint32 glhandle) const;
	virtual void Renderer::PushMatrix()const;
	virtual void Renderer::PopMatrix() const;
	virtual void Renderer::TranslateMatrix(double x, double y, double z) const;
	virtual void Renderer::RotateMatrix(double angle, double x, double y, double z) const; 
protected:
    Renderer() {}
private:
